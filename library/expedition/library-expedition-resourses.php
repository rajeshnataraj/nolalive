...
$qrymedias = $ObjDB->QueryObject("SELECT fld_media_name, fld_media_file_type, fld_media_file_name, fld_media_desc, fld_id
									...
									WHERE fld_exp_dest_task_id='".$taskid."' AND fld_media_category='3' AND fld_flag='1' AND fld_delstatus='0'");
?>
<section data-type='2home' id='library-expedition-resourses'>
    <div class='span12 dialogStyle1'>
        <div class='row'>
            <div class='twelve columns'>
            	<span style="color: <?php if($sessmasterprfid==10) { ?> #FFFFFF;<?php } else {?> #537F98;<?php }?> font-family: 'source_sans_proregular'; font-size: 18px; font-weight: bold;">Task <?php echo $taskorder;?>:</span>
        ...
        <div class='row-fluid'>
            ...
            <div class='row buttons'>
                <p style="color: <?php if($sessmasterprfid==10) { ?> #FFFFFF;<?php } else {?> #537F98;<?php }?> font-family: 'source_sans_proregular'; font-size: 18px; font-weight: bold; margin-bottom:7px;">Resources:</p>
                <?php
                $qryresources= $ObjDB->QueryObject("SELECT fld_id, fld_res_name, fn_shortname (CONCAT(fld_res_name), 1) AS shortname, fld_res_file_name, fld_res_file_type, fld_res_status, fld_typeof_res FROM itc_exp_resource_master WHERE fld_task_id='".$taskid."' AND fld_flag='1' AND fld_delstatus='0' ORDER BY fld_order");
                if($qryresources->num_rows>0) {
					...
					while($rowqqryresources = $qryresources->fetch_object()){
						...
						$i++;
                          ...
                          $select_viewexpmatlist=$ObjDB->QueryObject("SELECT AB.fld_material as matid,GH.fld_materials as materialname,GH.fld_thumbimg_url as thumbimg,GH.fld_catalog_url as catalogurl FROM itc_exp_extendmaterials_mapping AS AB
                                                                           INNER JOIN itc_materials_master AS GH ON AB.fld_material = GH.fld_id WHERE
                                                                           AB.fld_expedition='".$expeditionid."' AND AB.fld_destination='".$destinationid."' AND AB.fld_task='".$taskid."' AND AB.fld_delstatus='0'");
                        ...
                        if($select_viewexpmatlist->num_rows > 0)   {
                                        ...
                                        ?>
                 <a class="skip btn mainBtn" href="#library-expedition-viewmaterialfortask" id="btnlibrary-expedition-viewmaterialfortask" name="<?php echo $expeditionid;?>,<?php echo $destinationid;?>,<?php echo $taskid; ?>">
