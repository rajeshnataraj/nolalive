...
* @package Utilities
**/
$domainame = "http://".$_SERVER['SERVER_NAME']."/";
$content_url = CONTENT_URL;

header("Cache-Control: no-cache, must-revalidate");
header("Expires: ".date("D, d M Y H:i:s e")."");

if($domainame == "http://localhost/") {
	$domainame = "http://".$_SERVER['SERVER_NAME']."/synergy-itc/";
	define('__EXACTPATH__', $_SERVER['DOCUMENT_ROOT']."/synergy-itc/");
	$content_url = "http://localhost";
}
else {
	define('__EXACTPATH__', $_SERVER['DOCUMENT_ROOT']."/");
}

define('__HOSTADDR__', $domainame);
define('__LDPATH__', '');
define('__REPORT__','http://54.148.209.47/'); //Report server ip
define('__CNTPATH__', CONTENT_URL .'/'); /***all uploading file folder****/
define('_CONTENTURL_', CONTENT_URL.'/');
define('__CNTICONPATH__', _CONTENTURL_.'iplicon/');     // for the purpose of storing the lesson icon images in this iplicon folder
define('__CNTUNITICONPATH__', _CONTENTURL_.'uniticon/'); // for the purpose of storing the units icon images in this uniticon folder
define('__CNTCOURSEICONPATH__', _CONTENTURL_.'courseicon/'); // for the purpose of storing the course icon images in this uniticon folder
define('__CNTPDICONPATH__', _CONTENTURL_.'pdlessonicon/'); // for the purpose of storing the pdlesson icon images in this uniticon folder
define('__CNTMATERIALICONPATH__', _CONTENTURL_.'materialicon/'); // for the purpose of storing the units icon images in this materialicon folder
define('__CNTACTIVITYPATH__',_CONTENTURL_.'activity/'); // for the purpose of storing files related to acitvity
define('__CNTPPPATH__', _CONTENTURL_.'profile-pic/');// for the purpose of storing the profile picture
define('__CNTSLPATH__', _CONTENTURL_.'school-logo/');//  for the purpose of storing the school-logo
define('__CNTASSETPATH__',_CONTENTURL_.'asset/');
define('__CNTANSIMGPATH__',_CONTENTURL_.'question/ansimg/');
define('__CNTDRAGNDROPPATH__',_CONTENTURL_.'question/draganddrop/');
define('__CNTQUEIMGPATH__',_CONTENTURL_.'question/images/');
define('__CNTREMPATH__',_CONTENTURL_.'question/remediations/');
//define('__CNTREMPATH__',_CONTENTURL_.'question/remediations/');
...
if($domainame == "http://localhost/synergy-itc/") {
	define('DB_CONNECTION','server=localhost;database=synergyitc;username=root;password=root;port=3306;socket=');
}
	...
	*/
	public function __construct($ConnectionString, $ThrowExceptions = true)
	{
		...
		@$this->connect(empty($CS['server']) ? 'localhost' : $CS['server'],
							 ...
							 empty($CS['database']) ? 'synergyitc' : $CS['database'],
